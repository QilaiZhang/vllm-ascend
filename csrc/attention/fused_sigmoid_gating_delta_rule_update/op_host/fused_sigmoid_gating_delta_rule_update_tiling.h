...
/*!
 * \file fused_sigmoid_gating_delta_rule_update_tiling.h
 * \brief
 */
#ifndef __OP_HOST_FUSED_SIGMOID_GATING_DELTA_RULE_UPDATE_TILING_H__
#define __OP_HOST_FUSED_SIGMOID_GATING_DELTA_RULE_UPDATE_TILING_H__
#include <tiling/tiling_api.h>
#include "register/tilingdata_base.h"
#include "tiling_base/tiling_base.h"
#include "tiling_base/error_log.h"
#include "../op_kernel/fused_sigmoid_gating_delta_rule_update_tiling_data.h"

namespace optiling {
using namespace FusedSigmoidGatingDeltaRuleUpdate;

struct FusedSigmoidGatingDeltaRuleUpdateCompileInfo {
    uint64_t aivNum{0UL};
    uint64_t ubSize{0UL};
};

struct FusedSigmoidGatingDeltaRuleUpdateInfo {
public:
    int64_t usedCoreNum = 0;
    const char *opName = "FusedSigmoidGatingDeltaRuleUpdate";
};

class FusedSigmoidGatingDeltaRuleUpdateTiling : public Ops::Transformer::OpTiling::TilingBaseClass {
public:
    explicit FusedSigmoidGatingDeltaRuleUpdateTiling(gert::TilingContext *context) : Ops::Transformer::OpTiling::TilingBaseClass(context)
    {
        InitCompileInfo();
    };
    ~FusedSigmoidGatingDeltaRuleUpdateTiling() override = default;
...
protected:
    ...
    using HostRuleFn = ge::graphStatus (FusedSigmoidGatingDeltaRuleUpdateTiling::*)();
    ...
    FusedSigmoidGatingDeltaRuleUpdateCompileInfo compileInfo_;
    FusedSigmoidGatingDeltaRuleUpdateTilingData tilingData_;
    FusedSigmoidGatingDeltaRuleUpdateInfo inputParams_;
    UbCalcContext ubCalcCtx_;
};

} // namespace optiling
#endif // __OP_HOST_FUSED_SIGMOID_GATING_DELTA_RULE_UPDATE_TILING_H__
