...
def _causal_conv1d_update_kernel(
    ...
    if USE_PAD_SLOT:  # noqa
        if conv_state_input_coord == pad_slot_id:
            # not processing as this is not the actual sequence
            return
    ...
    conv_states_base = (conv_state_ptr +
                        (conv_state_input_coord * stride_conv_state_seq) +
    ...
    conv_state_ptrs_source = (
        conv_state_ptr + (conv_state_input_coord * stride_conv_state_seq) +
        conv_state_token_offset * stride_conv_state_tok +
        (idx_feats * stride_conv_state_dim)[None, :] +
        ((idx_tokens + 1) * stride_conv_state_tok)[:, None]
    )  # [BLOCK_M, BLOCK_N]
    mask = ((conv_state_input_coord < num_cache_lines)
    ...
    conv_states_offset = tl.load(conv_state_indices_ptr +
                                 idx_seq * stride_state_indices + current_last_index)
    conv_state_base = (conv_state_ptr +
                       (conv_states_offset * stride_conv_state_seq) +
                       (idx_feats * stride_conv_state_dim))  # [BLOCK_N,]
    conv_state_ptrs_target = (conv_state_base[None, :] +
